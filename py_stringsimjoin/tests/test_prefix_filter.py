...
class FilterPairTestCases(unittest.TestCase):
    ...
    def test_jac_dlm_08_prune(self):
        self.test_filter_pair('aa bb cc dd ee', 'xx yy cc dd ee',
                              self.dlm, 'JACCARD', 0.8, False, True)

    def test_jac_dlm_08_pass(self):
        self.test_filter_pair('aa bb cc dd ee', 'xx aa cc dd ee',
                              self.dlm, 'JACCARD', 0.8, False, False)

    # tests for COSINE measure 
    def test_cos_dlm_08_prune(self):
        self.test_filter_pair('aa bb cc dd ee', 'xx yy cc dd ee',
                              self.dlm, 'COSINE', 0.8, False, True)

    def test_cos_dlm_08_pass(self):
        self.test_filter_pair('aa bb cc dd ee', 'xx aa cc dd ee',
                              self.dlm, 'COSINE', 0.8, False, False)

    # tests for DICE measure 
    def test_dice_dlm_08_prune(self):
        self.test_filter_pair('aa bb cc dd ee', 'xx yy cc dd ee',
                              self.dlm, 'DICE', 0.8, False, True)

    def test_dice_dlm_08_pass(self):
        self.test_filter_pair('aa bb cc dd ee', 'xx aa cc dd ee',
                              self.dlm, 'DICE', 0.8, False, False)

    # test allow_missing flag
    def test_prefix_filter_pass_missing_left(self):
        self.test_filter_pair(None, 'fg ty',
                              self.dlm, 'DICE', 0.8, True, False)
    ...
    def test_empty_lstring(self):
        self.test_filter_pair('ab', '', self.dlm, 'JACCARD', 0.8, False, True)

    def test_empty_rstring(self):
        self.test_filter_pair('', 'ab', self.dlm, 'JACCARD', 0.8, False, True)

    def test_empty_strings(self):
        self.test_filter_pair('', '', self.dlm, 'JACCARD', 0.8, False, True)


    @nottest
    def test_filter_pair(self, lstring, rstring, tokenizer, sim_measure_type,
                         threshold, allow_missing, expected_output):
...
class FilterTablesTestCases(unittest.TestCase):
    ...
    def test_cos_dlm_08(self):
        expected_pairs = set(['1,5', '4,2', '5,1'])
    ...
    def test_dice_dlm_08_with_njobs_above_1(self):
        expected_pairs = set(['1,5', '4,2', '5,1'])
        self.test_filter_tables(self.dlm, 'DICE', 0.8, False,
                                (self.A, self.B,
                                'id', 'id', 'attr', 'attr',
...
class FilterCandsetTestCases(unittest.TestCase):
    ...
    def test_jac_dlm_08_allow_missing(self):
        expected_pairs = set(['1,5',
                              '6,1', '6,2', '6,3', '6,4', '6,5',
                              '6,6', '1,6', '2,6', '3,6', '4,6', '5,6'])
        self.test_filter_candset(self.dlm, 'JACCARD', 0.8, True,
                                (self.C, 'l_id', 'r_id',
                                 self.A, self.B,
                                'l_id', 'r_id', 'l_attr', 'r_attr'),
                                expected_pairs)
...
class PrefixFilterInvalidTestCases(unittest.TestCase):
    ...
    def test_invalid_ltable(self):
        prefix_filter = PrefixFilter(self.tokenizer, self.sim_measure_type,
                                     self.threshold)
        prefix_filter.filter_tables([], self.B, 'A.id', 'B.id',
                                    'A.attr', 'B.attr')
    ...
    def test_invalid_l_key_attr(self):
        prefix_filter = PrefixFilter(self.tokenizer, self.sim_measure_type,
                                     self.threshold)
        prefix_filter.filter_tables(self.A, self.B, 'A.invalid_id', 'B.id',
                                    'A.attr', 'B.attr')
    ...
    def test_invalid_r_key_attr(self):
        prefix_filter = PrefixFilter(self.tokenizer, self.sim_measure_type,
                                     self.threshold)
        prefix_filter.filter_tables(self.A, self.B, 'A.id', 'B.invalid_id',
                                    'A.attr', 'B.attr')
    ...
    def test_invalid_threshold(self):
        prefix_filter = PrefixFilter(self.tokenizer, self.sim_measure_type, 1.2)
