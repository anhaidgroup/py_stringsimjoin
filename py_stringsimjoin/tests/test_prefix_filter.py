...
class FilterPairTestCases(unittest.TestCase):
    def setUp(self):
        ...
        self.prefix_filter = PrefixFilter(self.tokenizer,
                                          sim_measure_type='JACCARD',
                                          threshold=0.8)

    def test_to_be_pruned_pair(self):
        self.assertTrue(self.prefix_filter.filter_pair('aa bb cc dd ee',
                                                       'xx yy cc dd ee'))

    def test_to_be_passed_pair(self):
        self.assertFalse(self.prefix_filter.filter_pair('aa bb cc dd ee',
                                                        'xx aa cc dd ee'))
...
class FilterTablesTestCases(unittest.TestCase):
    ...
    def test_valid_tables(self):
        ...
        C = self.prefix_filter.filter_tables(A, B,
                                           'id', 'id',
                                           'attr', 'attr')
...
class FilterCandsetTestCases(unittest.TestCase):
    ...
    def test_valid_candset(self):
        ...
        # TODO(paulgc) : This needs to be changed based on how token ordering is computed.
        expected_pairs = set(['1,5'])

        D = self.prefix_filter.filter_candset(C,
                                              'l_id', 'r_id',
                                              A, B,
                                              'l_id', 'r_id',
                                              'l_attr', 'r_attr')
