...
class FilterPairTestCases(unittest.TestCase):
    def setUp(self):
        ...
        self.suffix_filter = SuffixFilter(self.tokenizer,
                                          sim_measure_type='JACCARD',
                                          threshold=0.8)

    def test_to_be_pruned_pair(self):
        self.assertTrue(self.suffix_filter.filter_pair('aa bb cc dd ee',
                                                       'xx yy cc zz ww'))

    def test_to_be_passed_pair(self):
        self.assertFalse(self.suffix_filter.filter_pair('aa bb cc dd ee',
                                                        'xx aa cc dd ee'))

    def test_empty_input(self):
        self.assertTrue(self.suffix_filter.filter_pair('ab', ''))
        self.assertTrue(self.suffix_filter.filter_pair('', ''))
...
class FilterCandsetTestCases(unittest.TestCase):
    ...
    def test_valid_candset(self):
        ...
        expected_pairs = set(['1,5', '3,4'])

        D = self.suffix_filter.filter_candset(C,
                                              'l_id', 'r_id',
                                              A, B,
                                              'l_id', 'r_id',
                                              'l_attr', 'r_attr')
